...
void Graph::updateConnection(int v, int u, double w) {
    if (/* v is out of bounds — check if v is a valid index into nodes */ v < 0 || v >= nodes.size()) {
        cerr << "Attempting to update connection between " << v << " and " << u << " with weight " << w << " but " << v << " does not exist" << endl;
        exit(1);
    }
    if (/* u is out of bounds — check if u is a valid index into nodes */ u < 0 || u >= nodes.size()) {
